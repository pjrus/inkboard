...
describe("export bounds", () => {
  ...
  it("pads the content bounds and returns null for an empty board", () => {
    const b = contentBounds([stroke({ minX: 10, minY: 20, maxX: 30, maxY: 40 })], 5)!;
...
describe("export page planning", () => {
  const board: CanvasObject[] = [stroke({ minX: 0, minY: 0, maxX: 400, maxY: 300 }), text(50, 400)];
  ...
  it("paginates tall content onto A4 pages that tile without gaps", () => {
    ...
    expect((pages[0].source.maxX - pages[0].source.minX) * pages[0].scale).toBeCloseTo(A4_PORTRAIT.width - 72);
  });

  it("chooses landscape for wide content on auto", () => {
    const wide = [stroke({ minX: 0, minY: 0, maxX: 4000, maxY: 500 })];
    expect(planA4Pages(wide, "auto", 0)[0].pageWidth).toBeCloseTo(A4_PORTRAIT.height);
    expect(planA4Pages(wide, "portrait", 0)[0].pageWidth).toBeCloseTo(A4_PORTRAIT.width);
  });

  it("emits one output page per imported PDF page, at the page's own size", () => {
    const objects: CanvasObject[] = [pdfPage(1, 0), pdfPage(2, 840), stroke({ minX: 10, minY: 10, maxX: 100, maxY: 100 })];
    const pages = planPDFPages(objects);
    expect(pages).toHaveLength(2);
    expect(pages[0]).toMatchObject({ pageWidth: 612, pageHeight: 792, scale: 1 });
  ...
  it("gives content outside every imported page its own page rather than dropping it", () => {
    const objects: CanvasObject[] = [pdfPage(1, 0), stroke({ minX: 2000, minY: 2000, maxX: 2100, maxY: 2100 })];
    const pages = planPDFPages(objects);
    expect(pages).toHaveLength(2);
...
describe("export rendering helpers", () => {
  ...
  it("draws pages, then ink, then text - and only what touches the page", () => {
    ...
    expect(objectsOnPage(objects, geometry).map((o) => o.type)).toEqual(["pdf-page", "stroke", "text"]);
  });
...
describe("export filenames", () => {
  it("keeps readable names and strips characters filesystems reject", () => {
    ...
    expect(pdfFileName("Lecture 5: intro/outro")).toBe("Lecture 5 intro outro.pdf");
