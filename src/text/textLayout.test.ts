...
import { firstBaseline, layoutText, lineHeightFor, wrapLines } from "./textLayout";
...
describe("text layout", () => {
  it("wraps greedily at word boundaries", () => {
    ...
    expect(wrapLines("hello world again", 100, measure(20))).toEqual(["hello", "world", "again"]);
    expect(wrapLines("hello world again", 160, measure(20))).toEqual(["hello world", "again"]);
  });
  ...
  it("height follows the line count, not the box", () => {
    const result = layoutText("hello world again", { width: 100, fontSize: 20, fontFamily: "open-sans" }, measure(20));
  ...
  it("an empty box still occupies one line", () => {
    expect(layoutText("", { width: 300, fontSize: 20, fontFamily: "open-sans" }, measure(20)).height).toBeCloseTo(lineHeightFor(20));
  });
  ...
  it("places lines for each alignment", () => {
    ...
    const centre = layoutText("abcd", { ...opts, align: "center" }, measure(20));
  ...
  it("centres the font's own ascent and descent inside the line box", () => {
    ...
    const size = 20;
    const expected = (lineHeightFor(size) - (f.ascent - f.descent) * size) / 2 + f.ascent * size;
    expect(firstBaseline("open-sans", size)).toBeCloseTo(expected);
    // Baselines are one line height apart, whatever the family.
    const layout = layoutText("a\nb", { width: 500, fontSize: size, fontFamily: "roboto" }, measure(size));
    expect(layout.lines[1].baseline - layout.lines[0].baseline).toBeCloseTo(lineHeightFor(size));
  });

  it("scales with font size, so world-space sizing zooms cleanly", () => {
    const small = layoutText("hello world", { width: 100, fontSize: 10, fontFamily: "inter" }, measure(10));
    const large = layoutText("hello world", { width: 200, fontSize: 20, fontFamily: "inter" }, measure(20));
    expect(large.lines.map((l) => l.text)).toEqual(small.lines.map((l) => l.text));
